...
`articles/{category}/{slug}.md` の形で記事を整理済み（現在249件）。`category` は14種類（`ai`, `fsharp`, `haskell`, `javascript`, `languages`, `math`, `media`, `misc`, `python`, `quantum`, `retro`, `tools`, `webassembly`, `wiktionary`）。ファイル名の英字slugはタイトル・本文から意訳した短いkebab-case。
