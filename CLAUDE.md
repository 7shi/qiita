...
- Qiita へ投稿・更新したら、取得し直して実際の値で埋める。
  ...
  diff で `updated_at` が空文字列から実際の日時に変わっている場合は、
  この「取得し直し」による正常な変化であり、反映漏れではない。
...
**ダッシュ（`——`・`—`・`–`）は使わない。** 文を切って句点で終える、読点でつなぐ、
括弧に入れる、箇条書きにするなど、通常の日本語の区切りで書き直す。

- 例: `平坦な 1 列に変わります——線形化です` → `平坦な 1 列に変わります。これが線形化です`

## 「要ります」を使わない

**「要ります」「要る」は使わない。「必要です」「必要な」に言い換える。**
口語的すぎて記事の文体に合わないため。

- 例: `` `Category` のインスタンスも要ります `` → `` `Category` のインスタンスも必要です ``
- 例: `` 定義が要るのは `left` だけです `` → `` 定義が必要なのは `left` だけです ``

**否定形「要らない」には「不要」を使う。**「必要ない」ではなく「不要」で統一する。

- 例: `` `Functor` インスタンスが要らない `` → `` `Functor` インスタンスが不要 ``
- 例: `数値リテラルだけ注釈が要らない理由` → `数値リテラルだけ注釈が不要な理由`

## 記事本文の強調

**ボールド（`**〜**`）は新出用語の定義箇所にのみ使う。** 主張を強調したい・重要な一文だから、
という理由では使わない。多用すると本当に覚えてほしい用語が埋もれるため。

- 例: `**Free モナド**は ~ です` のように、その語を初めて導入して説明する箇所。
- 強調したい内容は、文の構成や配置（節の冒頭・末尾に置く、箇条書きにする）で伝える。
- このルールは記事本文が対象。`README.md`・`*-PLAN.md` などの作業用ドキュメントは対象外。
