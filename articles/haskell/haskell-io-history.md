...
この問題は後に Clean が**一意型**（uniqueness type）で解決しました。そして皮肉なことに、**現在の GHC の IO はこの世界渡しで実装されています**。`IO` を `newtype` で包んで外から触れなくすることで、単一スレッド性を型システムではなく抽象化によって守っています。
...
* 世界渡しは**単一スレッド性を保証できず**却下された。現在の GHC はこの方式を `newtype` で隠して使っている。
