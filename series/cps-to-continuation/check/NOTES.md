...
対象: `series/cps-to-continuation/03-cps-to-continuation-monad.md`
...
エラーにならず、`evalCont` で `r` をレコード型に固定した瞬間に出る（`check/GenInfer.hs`）:
...
これで JS 版をほぼ一対一に移植できる（`check/GenNaive.hs`、実行結果 `1 2 3`）。
...
- `check/GenStd.hs` — 継続モナドは標準の `Control.Monad.Trans.Cont` を使い、
  コルーチンの部分だけを実装したもの。
- `check/GenMin.hs` — 継続モナドも含めて全部自前の最小実装。
...
詳細は単発記事 [`../haskell-generator.md`](../haskell-generator.md)
（継続モナドによるジェネレーターを Haskell で書く）に分離した。構成は

1. `# 移植元` — JS 版と Haskell の `Cont` の再掲
2. `# type だけだと循環する` — 型シノニムの循環エラー、occurs check
3. `# data を使うと解決する` — `data` は名前で型を導入するので循環を通せる。移植コードと実行結果
4. `# 直和型による整理` — `Gen`/`Done`/`Yield` による整理版、無限ジェネレーター
5. `# まとめ`
...
※ `haskell-generator.md` は未投稿のため、フロントマターの `id` と `url` が空。
投稿したら埋めること。`03-...md` からのリンクも相対パスのままなので、
