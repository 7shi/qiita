...
`03-cps-to-continuation-monad.md` に「# Haskell への移植」節を新設し、
以下の流れで説明を追加した。

1. `## type だけだと循環する` — 型シノニムの循環エラー、occurs check
2. `## data を使うと解決する` — `data` は名前で型を導入するので循環を通せる。移植コードと実行結果
3. `## 直和型による整理` — `Gen`/`Done`/`Yield` による整理版、無限ジェネレーター

併せて「移植できない」としていた 2 箇所を修正した。

- `# Haskell と Scheme` 節: 「型が循環してエラーになるため移植できません」
  → 「型が循環しますが、`data` で包めば移植できます」＋新設節へのリンク
- `## 移植元` 節: 「Haskell では実装していません」
  → 「節を改めて説明します」
