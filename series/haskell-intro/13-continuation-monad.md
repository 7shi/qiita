...
そこで、値の代わりに `k` の呼び出しを含む CPS の関数、つまり「継続に値を渡す関数」をモナドの中身にすることを考えます。関数は作っただけでは実行されないため、呼ぶタイミングが制御できます。これが継続モナド `Cont` です。
...
`Cont` の定義には `k` と `c` という 2 つの関数が現れます。どちらも継続と呼ばれますが、型が違うことに注意が必要です。

| | 型 | 中身 |
|---|---|---|
| `k`（bind に渡す関数） | `a -> Cont r b` | 続きの計算をモナドとして書いたもの |
| `c`（外側の `cont` が受け取る継続） | `b -> r` | 答えを受け取るだけの素の関数 |

`m`・`k`・`m >>= k` の型を並べると、bind によって `Cont r a` の `a` が `b` に変わる様子が見えます。

```hs
(m :: Cont r a) >>= (k :: a -> Cont r b) :: Cont r b
```

`m >>= k` 全体は `cont $ \c -> ...` という形の継続モナドです。`runCont (m >>= k) c` として呼び出したとき、その `c` がラムダの引数として渡ってきます。つまり `c` は bind の外から後で渡されるものです。

一方、bind の実装自身は `m` を剝がすために、その場で `runCont m` を呼んでいます。ここで要求される `a -> r` は、`k` とは戻り値の型が異なります。そのため bind の実装内で調整が必要となります。

bind の時点では `m` から値が取り出されていないため、`\x -> ...` によって、`runCont` が呼ばれたときに取り出された値を受け取ります。

```hs:型の調整
\x -> runCont (k x) c :: a -> r
```

このように `k` を `a -> Cont r b` から `a -> r` に変換することで、`runCont m` に渡せるようになります。

```hs:m から取り出した値を k に渡す
runCont m (\x -> runCont (k x) c) :: r
```

bind の時点では継続が定まっていないため、`\c -> ...` によって、`runCont` が呼ばれたときに外部から与えられた継続を受け取ります。

```hs:c を受け取る関数にする
\c -> runCont m (\x -> runCont (k x) c) :: (b -> r) -> r
```

`Identity` では bind の時点で `k` を評価して得られた値を保持していましたが、`Cont` では `k` を変換して得られた継続を関数の形で保持しているわけです。

## 継続を保持できることから3つの自由が出る

bind が CPS の形をしているのは `Cont` に限らずすべてのモナドに共通する性質です(83〜85行目)。それでも「継続モナド」という名前がつくのは、他のモナドの `k :: a -> m b` が「その効果の中で続きの計算を作るだけ」なのに対し、`Cont` では `k` の呼び出しが `cont $ \c -> ...` という関数の中に閉じ込められ、「ここから先の計算全体」を表す値として保持できるからです。
...
`x == 0` のとき `ret "zero"` を呼ぶと、`return "non-zero"` には到達せず `"zero"` が結果になります。この動作がどのように実現されているのかを、以下で組み立てていきます。

## 実装の組み立て

`callCC` もモナドを返すため `cont $ \c -> ...` という形になります。この `c` が現在の継続です。`f` はモナドを返すので、`runCont` で剝がして `c` を渡せば答えが得られます。

```hs:骨組み
callCC f = cont $ \c -> runCont (f ?) c
```

残るのは `f` に渡す `?` の部分です。取り出したい継続は `c` ですが、`c` は `a -> r` なので、そのままでは `do` の中で bind につなげられません。bind に渡す `k` と同じ `a -> Cont r b` に変換する必要があります。bind とは変換の向きが逆です。

| | 変換するもの | 変換後 | 実装 |
|---|---|---|---|
| bind | `k`（`a -> Cont r b`） | 素の継続 `a -> r` | `\x -> runCont (k x) c` |
| `callCC` | `c`（`a -> r`） | `k` と同じ型 | `\x -> cont $ \_ -> c x` |

`\x ->` で値を受け取って `c x` で答えを得て、それを `cont` で包めばモナドになります。このとき包んだ関数の引数は後続の継続を受け取りますが、`\_ ->` として捨てます。

つまり後続を飛ばす処理は、取り出される継続そのものに埋め込まれています。これで実装が得られます。

```hs
callCC f = cont $ \c -> runCont (f (\x -> cont $ \_ -> c x)) c
```

`f` に渡される `\x -> cont $ \_ -> c x` が「現在の継続」、つまり先ほどの `ret` の正体です。取り出されるのは `k` と同じ型なので、`do` の中でそのまま使えます。

実装には `c x` と末尾の `c` の 2 箇所で `c` が現れるため二重に適用しているように見えますが、先ほどの 2 例はどちらか一方だけを通ります。

| 例 | 結果 | `c` を適用する箇所 |
|---|---|---|
| `f 0`（`ret` を呼ぶ） | `"zero"` | `c x` |
| `f 1`（`ret` を呼ばない） | `"non-zero"` | 末尾の `c` |

末尾の `c` は、bind で結合された後続の末尾に埋め込まれた形で使われます。`ret "zero"` を呼ぶと `\_ -> c x` がその継続を丸ごと無視するため、末尾の `c` には到達しません。これが「脱出」の実体です。

先ほどの型で `a -> Cont r b` となっていた `b` は、`ret` の呼び出し側が自由に選べる型変数です（本体の中では1つの型に決まります）。型の上では `ret` の戻り値を受け取って使えてしまいますが、実行時には `\_ -> c x` が後続を捨てるため、そこには到達しません。このように「呼んだら戻らない」性質を**abortive**（脱出専用）と呼びます。「戻らない」ことを保証しているのは型ではなく、この実装です。
