...
`Identity` の bind では、継続 `k` はその場で呼ばれるだけです。呼び出し元が渡した継続を、モナドの中に保持しておいて後で取り出す、ということはできません。

そこで発想を変えます。値をそのまま持つのではなく、計算そのものを CPS で表現してしまいます。

順序に注意してください。継続モナドが `>>=` とうまく噛み合うのは偶然ではありません。もともと `>>=` の型が継続渡しの形をしていて、どのモナドもその骨格の上に乗っています。継続モナドはその骨格を値として保持できるようにしただけで、新しい仕組みを持ち込むわけではありません。
...
| `c`（外側の `cont` が受け取る受け手） | `b -> r` | 答えを受け取るだけの素の関数 |

`m >>= k` 全体の型は `Cont r b` なので、外側の `cont $ \c -> ...` が受け取る `c` は結果の型 `b` 側、つまり `b -> r` です。一方 `runCont m` が要求するのは `a -> r` です。どちらも「`_ -> r` という素の関数」ですが、型引数が違います。

`k` は素の関数ではないので、そのままでは `runCont m` に渡せません。`k x` でモナドを得て、`runCont` で剝がして `c` につなげば型が合います。

```hs:型を合わせた継続
\x -> runCont (k x) c :: a -> r
```

つまり bind は、`k` を `c` と同じ形の素の継続に変換して `m` に渡す操作です。`Identity` では `k` をその場で呼ぶだけでしたが、`Cont` では変換した継続が関数の形で `Cont` の中に住んでいます。

外側の `cont $ \c -> ...` は、その `c` が bind の時点では決まっておらず、`runCont` が呼ばれたときに渡されるための受け皿です。
