...
推敲完了（2026-08-06 に構成案 8 の節を削除。決定事項 15。
...
同日、まとめの「`instance Monad` を書くのに必要なのは」を「モナドを自作するのに必要なのは」に修正し、
AMP の `:::message` にあったシリーズ中断への言及をまとめの末尾へ移した）。
Zenn へ複製し公開済み（2026-08-07、`ZENN.tsv` にも追記済み）。**
...
- [x] 推敲結果を「執筆時の変更点」「記事の実際の構成」へ反映する
...
- **まとめの「`instance Monad` を書くのに必要なのは、実質 `pure` と `>>=` の 2 つだけです」を
  「モナドを自作するのに必要なのは」に修正した**（2026-08-07・レビュー指摘）。
  `pure` は `Applicative` のメソッドであり `Monad` 自体の最小完全定義は `(>>=)` のみなので
  （`## Monad のスーパークラス` の `MINIMAL` 表示、`## Identity` の「単独では書けず」と整合）、
  主語を「`instance Monad` を書くのに」と絞ると `pure` が `Monad` 自身のメソッドであるかのように
  読めてしまう。元々は `## 3 段まとめて書く定型`（`Functor`/`Applicative`/`Monad` 3 つ揃えた
  `instance` 群）についての記述だったので、3 段まとめて指す言い方に戻した。
- **AMP の `:::message` にあった「シリーズは途中で 10 年以上中断していた」という 1 文を
  まとめの末尾へ移した**（2026-08-07・ユーザー指示）。AMP の説明自体は仕様の歴史の話だが、
  中断の経緯はシリーズ全体を振り返る文なので、AMP のメッセージ内よりまとめの結びに置く方が
  収まりが良い。
