...
`fmap`（`<$>`）は 1 引数の関数を `f` の世界へ持ち上げました。引数が 2 つある関数を同じように持ち上げてみます。
...
当時は演算子の使い方だけを示しましたが、その正体は `Functor`・`Applicative` のメソッドだったわけです。
...
これは「関数に渡されるのはモナドではなく素の値なので、関数側に `Monad m =>` を書かなくてよい」という意味でした。関数側には要りませんが、`<$>`・`<*>` を使う側には制約が付きます。
