...
- **未定のアイデア**: アロー（19回）まで完了したら、以下のような理論寄りの話題を扱う回を追加することも考えられる。いずれも決定ではないので、書くかどうかはその時点で議論する。**共通する方針は「理論から Haskell を説明する通常の向きではなく、シリーズで積み上げた Haskell の道具立ての側から理論の概念を眺める逆向きの構成にする」こと。** シリーズ本編の「圏論には言及しない」方針（上記スタイル参照）は、あくまで入門として準備が重すぎるという理由なので、ひととおり終えた後に改めて扱うなら矛盾しない。
  ...
  - **パラメトリシティと自由定理**: `id :: a -> a` は恒等関数しかありえない、`map` の型だけから等式が導けるという話。型クラスの回で扱ったパラメトリック多相の直接の続きで、「型が仕様になる」という主張の裏付け。圏論の自然変換（`Functor` 間の多相関数が自然変換になる理由）ともつながる。
  - **F 代数と再帰スキーム（`Fix`・catamorphism）**: Free モナドの直接の親戚（`Free f` は `Fix` に「変数」を足した形）なので、Free を書いた後なら導入コストが小さい。「畳み込みを一般化する」という切り口はシリーズの実装志向と相性が良く、圏論の「代数」の具体例にもなる。
  ...
  - **カリー＝ハワード同型対応**: 型と命題、プログラムと証明の対応。代数的データ型（直積型＝論理積、直和型＝論理和、関数型＝含意）を扱った回の知識から入れる。
  - **ヒンドリー・ミルナー型推論**: Haskell が型注釈なしで型を決められる仕組み。単一化（unification）と let 多相。シリーズでは型推論の結果（GHCi の `:type`）を使い続けてきたので、その裏側を明かす位置づけ。
- **網羅性の穴（`Foldable`・`Traversable`）**: 本文で未扱い。15回が `Functor`・`Applicative`・`Monad` の3段で止まっているため。`traverse` は `Applicative` の最大の実用例なので、理論回というより15回の続編として補うのが自然。`Alternative`・`MonadFail` も同様に宙に浮いている（08回・09回・12回で先送りされたまま）。`Comonad` も未扱いだが、実用場面が限られるため優先度は低い。
