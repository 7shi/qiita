...
- **【TODO】セクション指定リンク。** 記事の特定の箇所に触れている場合は、最終的にアンカー付き（`URL#見出し`）のリンクにしたい。現状は記事トップへのリンクにしてある。
  - 見出しからアンカーへの変換規則（日本語見出しのパーセントエンコード、記号の扱いなど）は実例から確認済みで、**Qiita と Zenn で同一**だった。規則の詳細と実装は [scripts/anchor.py](../../scripts/anchor.py) の docstring を参照（実例は [anchor-sample.md](../../anchor-sample.md)）。`uv run scripts/anchor.py` で実例との照合を検証できる。
