...
- **【TODO】セクション指定リンク。** 記事の特定の箇所に触れている場合は、最終的にアンカー付き（`URL#見出し`）のリンクにしたい。現状は記事トップへのリンクにしてある。
  - 見出しからアンカーへの変換規則（日本語見出しのパーセントエンコード、記号の扱いなど）は実例から確認済みで、**Qiita と Zenn で同一**だった。規則の詳細と実装は [scripts/anchor.py](../../scripts/anchor.py) の docstring を参照（実例は [anchor-sample.md](../../anchor-sample.md)）。`uv run scripts/anchor.py --test` で実例との照合を検証できる。見出しを直接アンカーへ変換するには `uv run scripts/anchor.py <見出し>` を使う。
  - 残作業は、既存記事のリンクをアンカー付きに差し替えること。
...
- **未定のアイデア**: アロー（19回）まで完了したら、圏論を扱う回を追加することも考えられる。ただし圏論から Haskell を説明する通常の向きではなく、シリーズで積み上げた Haskell の道具立て（`Functor`・`Monad`・アローなど）の側から圏論の概念を眺める逆向きの構成にする。シリーズ本編の「圏論には言及しない」方針（上記スタイル参照）は、あくまで入門として準備が重すぎるという理由なので、ひととおり終えた後に改めて扱うなら矛盾しない。決定ではないので、書くかどうかはその時点で議論する。
- Freeモナドの「自由」の意味:
