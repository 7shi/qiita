...
19. [Haskell アロー 超入門](19-arrow.md)（Zenn で公開予定）
...
- **20回として「Haskell 圏論 超入門」を検討中**（2026-08-12）。下記「未定のアイデア」の圏論の項目を一回分に起こすもの。**19回の `Kleisli` がちょうど良い題材になる。**
  - 19回で `Kleisli` を `Category` のインスタンスとして実装として通っている。`newtype Kleisli m a b = Kleisli { runKleisli :: a -> m b }` を `>>>` でつなぐ経験があるので、Kleisli 圏（`a -> m b` を射とみなす圏）を後から名前として与えるだけで済む。
  - 同じく19回で `Category` そのもの（対象・射・合成・単位元）を扱っており、単位元が恒等関数とは限らない（`Kleisli` では `return`）ことも本文で説明済み。圏の公理をゼロから導入する必要がない。
  - **17回の Operational モナドと米田の補題**も題材になる。Operational の `instr :>>= k`（命令と継続の組）は Coyoneda（`data Coyoneda f a = forall b. Coyoneda (f b) (b -> a)`）と同じ形で、**任意の型構築子を `Functor` にしてしまう構成**にあたる。17回に書いた「Operational は `Functor` インスタンスが不要になる」（上記「Free・Operational・Effの関係」）の理由がこれ。既に書いたコードの形が先にあり、後から名前と理屈を与えられる。
  - **この構成には理由がある。** シリーズは「何でもかんでもモナドを圏論で説明すると、数学者には良くても普通の IT 技術者には敷居が高すぎる」という判断から、**意図的に他のプログラミング言語と同水準の扱い方をしてきた**（上記「動機」・スタイルの「圏論には言及しない」）。最後に、既に見てきた構造から取っ掛かりだけを見せる。**理論から入るのではなく具体例からの導入にする**のが狙い。
  - 「シリーズで積み上げた道具立ての側から理論を眺める逆向きの構成」（下記「未定のアイデア」の共通方針）に、最もきれいに乗る題材。
...
- **全称量化（`forall`）**: 09回で `runST :: (forall s. ST s a) -> a` を例に「明示的な全称量化」という用語だけ脚注で触れたが（`### forall` 節）、`RankNTypes` として掘り下げてはいない。Haskell の型が暗黙に全称量化されていること（`id :: a -> a` は実は `forall a. a -> a`）から出発し、量化の位置をずらすと表現力がどう変わるかを見せる。17回で存在型（GADTs 内の `forall b.`）を「`b` を外から見えなくする」道具として使った（そこも理屈には立ち入らなかった）ので、全称量化との対比としてここで補う。パラメトリシティの項目と対で扱うのが自然。
...
- **圏論**: よく言われる話題への最低限の導入に留める。Hask 圏（Haskell の型を対象、関数を射とみなす圏）、Kleisli 圏（`a -> m b` を射とみなす圏。`>=>` が合成にあたる）、「モナドは自己関手の圏におけるモノイド対象」の意味。シリーズで扱った `Functor`・`Monad`・アローが、これらの概念とどう対応するかという見方で説明する。**20回の候補として具体化しつつある**（上記「構想」参照）。
  - **入口は19回の `Category` と `Kleisli`。** 圏の公理（合成と単位元）は `Category` 型クラスそのものであり、`(->)` と `Kleisli m` という2つのインスタンスを既に書いている。抽象的な定義から始めず、この2つが同じ型クラスに収まったのはなぜかという問いから入れる。
  ...
  - **米田の補題は Operational（17回）から入る。** `instr :>>= k` が Coyoneda の形をしている（上記「構想」参照）。抽象的な補題の主張から始めず、書いたコードがなぜ `Functor` インスタンスなしで済んだのかという問いの答えとして出す。
  - Codensity / Yoneda による Free モナドの高速化（左結合 `>>=` が二次コストになる問題とその解法）も、圏論の道具が実際に効く数少ない例として一節に収まる。単独の回にはしない。上記の Coyoneda とは別の話（こちらは性能、あちらは `Functor` 導出）なので、混ぜない。
