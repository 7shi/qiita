...
- **【TODO】セクション指定リンク。** 記事の特定の箇所に触れている場合は、最終的にアンカー付き（`URL#見出し`）のリンクにしたい。ただし見出しからアンカーへの変換規則（日本語見出しのパーセントエンコード、記号の扱いなど）が Qiita と Zenn で異なる可能性があり、未確認のため現状は記事トップへのリンクにしてある。変換規則を確認したうえで差し替える。
