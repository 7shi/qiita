...
|ファイル|内容|
|---|---|
|`Gen.hs`|本文の掲載コード（`Free` の定義を先頭に補って連結）。`GenF`・`liftF`・`yield`・`Show`・`toList`|
...
- 1 行目は `Gen o a` の `Show` インスタンスで `count` の中身をそのままダンプしたもの。
  `do` が組み立てているデータの形が確認できた。`Show` はコンストラクターと同じ表記を
  出力するので、そのまま貼り付ければ同じ値になる。
- 2 行目は同じ `count` を `toList` で辿った結果。
