...
18 回の `freer-simple` パッケージの事前検証（2026-08-09）。

⚠ **記事には載らない。採らなかった案の記録。**
`freer-simple` は 2022-01-07 の 1.2.1.2 が最終リリースで現行 LTS に無いため、
18 回では `effectful` を使うことにした（18-PLAN「パッケージの変更」）。
**現行の検証コードは `check/18-effectful/` にある。**

以下の 3 本はすべて `effectful` へ移植済みで、**出力は 1 文字も変わらなかった。**
比較のために残してある。
