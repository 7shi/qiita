...
Haskell の解説で圏論の名前を見かけることがありますが、このシリーズは一貫して圏論に言及せずに進めてきました。最終回では順序を逆にして、これまで書いてきたコードの側から圏論の概念を眺めます。圏・関手・自然変換から「モナドは自己関手の圏におけるモノイド対象」まで、既に実装として通ってきたものに後から名前を与えていきます。
...
このシリーズは「圏論には言及しない」という方針で書いてきました。初期の回では導入文にも「圏論には言及しません」と明記しています。理由は、既にあるモナドを使う分には圏論の準備が不要だからです。準備に時間を掛けるより、書いて動かす方を優先しました。

最終回でその方針を解除します。ただし向きは逆のままです。

圏論から Haskell を導く解説は既に数多くあります。この記事はそれをしません。シリーズで書いてきたコードを先に置いて、それに後から名前を与えます。各節は Haskell のコードから始まり、定義は後から出てきます。
...
本記事では数式を使います。Haskell のコードで書けるものはコードで書き、対象と射のレベルの話や同型のように、コードにすると不自然になるものだけを数式にします。
...
逆向きに読むこともできます。モノイドとは、対象が 1 つしかない圏のことです。圏の方が広く、対象を 1 つに絞るとモノイドになります。この見方は後で効いてきます。

## Hask 圏

Haskell そのものを圏とみなしたものを $\mathbf{Hask}$ と呼びます。対象は Haskell の型、射は関数です。
...
このインスタンスが $\mathbf{Hask}$ にあたります。合成は関数合成、恒等射は恒等関数です。`Int` から `String` への射とは `Int -> String` という型の関数のことで、その関数が何本あってもすべて別の射です。

シリーズで書いてきた関数はすべて $\mathbf{Hask}$ の射です。これから出てくる関手も自然変換も、$\mathbf{Hask}$ の上で考えます。

## bottom

$\mathbf{Hask}$ には避けて通れない問題があります。Haskell のすべての型は `undefined` と、停止しない計算を要素として持ちます。これらをまとめて **bottom**（$\bot$）と呼びます。
...
圏を作るには「2 つの射が等しい」がはっきり決まっている必要がありますが、`seq` はその判定を壊します。そのため $\mathbf{Hask}$ は、厳密には圏になりません。

:::message
これは知られた問題で、$\mathbf{Hask}$ を圏として扱う議論はたいてい「bottom と `seq` を無視する」という前提を置いています。無視すれば対応はきれいに取れますし、実際のコードでも `seq` を使わなければ困りません。
...
関手は一般には圏 $\mathcal{C}$ から別の圏 $\mathcal{D}$ への対応ですが、`Functor` のインスタンスはすべて $\mathbf{Hask}$ から $\mathbf{Hask}$ への対応です。`Maybe Int` も `[Int]` も Haskell の型なので、行き先は $\mathbf{Hask}$ の中に留まります。

このように出発点と行き先が同じ圏である関手を**自己関手**（endofunctor）と呼びます。

型で見ると `f :: * -> *` という種がその現れです。型を受け取って型を返すので、$\mathbf{Hask}$ の対象から $\mathbf{Hask}$ の対象への対応になります。👉[Freeモナド](https://zenn.dev/7shi/articles/20260808-haskell-free-monad#種)
