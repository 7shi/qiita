...
$\mathbf{Set}$ は集合の圏です。対象が集合、射が集合から集合への写像で、合成は写像の合成、恒等射は何もしない写像です。Hask 圏の対象を型から集合に、射を関数から写像に置き換えたものだと思えば、形は同じです。

$\mathbf{Mon}$ はモノイドの圏です。対象がモノイド、射が**モノイド準同型**（monoid homomorphism）で、これは構造を保つ写像を指します。モノイド $m$ から $n$ への写像 $h$ が準同型であるとは、次の 2 つが成り立つことです。
...
準同型を合成しても準同型のままで、何もしない写像も準同型なので、$\mathbf{Mon}$ は圏になります。

:::message
$\mathbf{Mon}$ は、前に定義した `Mono` とは別物です。👉[一点圏](#一点圏)

`Mono` はモノイドを 1 つ決めて圏とみなしたもので、対象は `()` だけ、射がモノイドの元でした。$\mathbf{Mon}$ はモノイドそのものを対象として並べた圏です。モノイドが射の側にあるか対象の側にあるかという違いになります。
:::

## 忘却関手

モノイドは、集合に `<>` と `mempty` を備えたものでした。集合に付け加えられたこの演算と単位元が、モノイドの構造です。ここから構造を取り去って、元が集まっただけの集合に戻す対応を考えます。モノイド則は演算について述べるものなので、演算が消えれば一緒に消えます。`Sum Int` からは「`<>` が足し算、`mempty` が `Sum 0`」という情報が落ちて `Sum x` の形の値の集合が残り、`[a]` からは「`<>` が `++`、`mempty` が `[]`」が落ちてリストの集合が残ります。
...
$\mathrm{Hom}_{\mathbf{C}}(x, y)$ は、圏 $\mathbf{C}$ における $x$ から $y$ への射をすべて集めた集合です。**Hom 集合**（hom-set）と呼びます。Hom は homomorphism（準同型）の略で、代数で準同型全体を集めた記法を、圏論が射一般に広げて引き継いだものです。$\mathbf{Set}$ のように射が単なる写像である圏でも、名前は Hom のままです。同じ 2 つの対象でも、どの圏で考えるかによって射は変わるため、添え字で圏を指定します。両端の $x$・$y$ がその圏の対象でなければならない、という条件も添え字が決めています。

ここで集合 $a$ とモノイド $m$ を任意に取り、$a$ の側から $m$ の側へ向かう射を集めることを考えます。

$a$ は $\mathbf{Set}$ の対象、$m$ は $\mathbf{Mon}$ の対象なので、そのままでは同じ Hom 集合に置けません。片方を相手の圏へ移す必要があり、そこで $F$ と $U$ が使えます。

* $\mathbf{Mon}$ で考えるなら、$a$ を $F$ でモノイドにして $\mathrm{Hom}_{\mathbf{Mon}}(F a, m)$
* $\mathbf{Set}$ で考えるなら、$m$ を $U$ で集合にして $\mathrm{Hom}_{\mathbf{Set}}(a, U m)$
...
:::message
向きは $m$ の側から $a$ の側へ取ることもできます。その場合は $\mathrm{Hom}_{\mathbf{Mon}}(m, F a)$ と $\mathrm{Hom}_{\mathbf{Set}}(U m, a)$ を比べることになりますが、数えると合いません。

$a$ を `Bool`（`False`・`True` の 2 要素の集合）、$m$ を `Sum Int` とします。$F a$ は `Bool` を並べたリスト `[Bool]` です。

* $\mathrm{Hom}_{\mathbf{Set}}(U m, a)$ は、`Sum` の値それぞれに `False`・`True` を独立に割り当てた写像なので、膨大な数になります。
* $\mathrm{Hom}_{\mathbf{Mon}}(m, F a)$ は 1 つしかありません。`Sum 1 <> Sum (-1)` は単位元 `Sum 0` なので、準同型ならリスト側でも 2 つを連結して `[]` にならなければいけません。ところがリストは連結すると長さが足されるだけなので、どちらも `[]` しかありえません。同じ理由で `Sum n` の行先はすべて `[]` になり、`Sum` の値を潰す準同型 1 つだけが残ります。

片方は膨大、片方は 1 つだけで、対応の付けようがありません。$a \to m$ の向きなら数が合います。

* $\mathrm{Hom}_{\mathbf{Set}}(a, U m)$ は、`False` と `True` に `Sum` の値を 1 つずつ割り当てるので、値 2 つの組と同じだけあります。
* $\mathrm{Hom}_{\mathbf{Mon}}(F a, m)$ は、`[Bool]` からの準同型が `[False]` と `[True]` の行先だけで決まるので、やはり値 2 つの組と同じだけあります。

`Sum Int` に逆元があるのに対して、リストには連結して `[]` になる組がありません。この非対称が、向きを決めています。
...
左辺はモノイド準同型、右辺は単なる写像です。構造を保つ射と、構造を無視した写像が、同じだけあるということです。

対応の形は非対称です。$F$ は射の出発点にだけ、$U$ は行き先にだけ現れます。構造を作る側が出発点、構造を落とす側が行き先という置き方が決まっているので、$F$ と $U$ を入れ替えた対応は成り立ちません。このように 1 対 1 の対応で結ばれた $F$ と $U$ の関係を**随伴**（adjunction）と呼び、$\mathrm{Hom}$ の左側に現れる $F$ を**左随伴**、右側に現れる $U$ を**右随伴**と呼んで、$F \dashv U$ と書きます。左右は $\mathrm{Hom}$ での位置に付けた名前です。「自由」とはこの左側にいることを指しています。

加えて、この対応は $a$ と $m$ について自然であることが求められます。自然性の節で見たのと同じ意味で、$a$ や $m$ を射で動かしてから対応させても、対応させてから動かしても、同じところに着くということです。👉[自然性](#自然性)

## 普遍性

前節の 1 対 1 の対応を Haskell で見ると、`[a] -> m` の準同型と `a -> m` の関数が同じだけあるということです。この対応は「1 つ与えると一意に決まる」という形で現れます。
...
これが上の 1 対 1 の対応の片道です。逆向きもあります。準同型 `h :: [a] -> m` が与えられたら、`\x -> h [x]` で要素 1 つの移し方に戻せます。要素を 1 本のリストにする対応を挟むだけです。この 2 つが互いに逆になっていることが「一意に決まる」の中身で、随伴の条件そのものになっています。
...
シリーズの方針は「あまり数学に深入りせず、他のプログラミング言語と同じような接し方をする」でした。Haskell は数学的な背景から説明されることが多いですが、このシリーズは言語の機能として説明し、書いて動かすことを優先しました。

そしてある程度 Haskell に慣れた段階で、対応物があるものから圏論を導入しました。
...
|話題|概要|
|---|---|
|`Foldable`・`Traversable`|畳み込みと、効果を伴う走査。入口は `foldMap` と `traverse`。`traverse` は `Applicative` の最大の実用例|
|`Alternative`・`MonadFail`|失敗と選択。`empty` と `<\|>` で候補を並べ、`fail` でパターン照合の失敗を扱う|
|遅延評価の理論|必要になるまで計算しない評価戦略。空間リークの原因にもなる。入口は `foldl` と `foldl'`、`seq`、`BangPatterns`|
|パラメトリシティと自由定理|型が実装を縛る性質。自然変換の自然性条件もその一例。`id :: a -> a` は恒等関数しかありえない|
|カリー＝ハワード同型対応|型と命題、プログラムと証明の対応。代数的データ型（直積・直和・関数型）から入る|
|ヒンドリー・ミルナー型推論|注釈なしで型を決める仕組み。単一化と let 多相が中核|
|F 代数と再帰スキーム|再帰を関手の代数として扱う枠組み。`Free f` から変数を除くと `Fix f`|
|`ArrowLoop`・FRP|矢印にフィードバックを入れる枠組みと、時間変化する値の記述。入口は Yampa|
|Codensity による Free の高速化|左結合の `>>=` が二乗時間になる問題を、継続で吸収して解く|
|`Comonad`|モナドの双対。`extract` で文脈から値を取り出し、`duplicate` で文脈を複製する|
|STM など並行処理|共有状態をトランザクションで扱う並行計算|
