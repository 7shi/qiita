...
|`instance Category (Kleisli m)`|クライスリ圏|
...
`Category` のインスタンスなので、これも圏です。名前が付いています。**クライスリ圏**（Kleisli category）です。
...
対象は Hask と同じですが、射が違います。Hask で `a` から `b` への射は `a -> b` でしたが、クライスリ圏では `a -> m b` です。同じ対象の上に別の射を敷いた圏ということになります。

ここでも「`a` から `b` への射」は `b` を返す関数ではありません。返ってくるのは `m b` です。`b` は合成の辻褄を合わせるための名前として機能しています。

モナド `m` ごとにクライスリ圏が 1 つ決まります。`Kleisli Maybe` なら失敗する計算の圏、`Kleisli []` なら非決定性計算の圏です。
...
つまりモナド則とは「クライスリ圏が圏であること」の要求でした。モナドが満たすべき性質として天下り式に与えられていたものが、圏の側から見ると当たり前の要求だったことになります。
...
新しいコードは何もありません。前回書いた `Kleisli` のインスタンスに、クライスリ圏という名前が付いただけです。
...
:::message
`>>=` を基本に取った組（型構築子 `m`・`return`・`>>=`）を**クライスリトリプル**（Kleisli triple）と呼びます。`join` を基本に取った定義と同値で、クライスリ圏をそのまま与える形になっています。Haskell の `Monad` クラスはこちらの形です。
:::

### モナド則の書き換え

クライスリ圏の節で見た `>=>` 版のモナド則を、`join` の側に書き換えておきます。手掛かりは以下の定義です。
...
`f` と `g` を両方 `id` にすれば、`join` が現れます。モナド則がクライスリ射すべてについて成り立つため、`id` に置き換えることができます。クライスリ射は `a -> m b` の形ですが、`a` に `m b` を選べば `m b -> m b` になり、`id` の型と一致します。
...
満たすべき法則も対応しますが、`join` の形のままでは見比べにくいので、クライスリ圏の節で見た `>=>` 版のモナド則を使います。`>=>` を `<>` に、`return` を `mempty` に読み替えれば、モノイド則と同じ形であることが分かります。ただし `<>` が結合するのは値で、`>=>` が合成するのは射です。
...
|`instance Category (Kleisli m)`|クライスリ圏、モナド則は圏の公理|
...
プログラムがどう評価され、どういう意味を持つかという層です。最初の 2 つは Hask 圏の節で触れた bottom の背景にあたります。
...
|計算のモナドとクライスリトリプル|副作用のある計算に意味を与える枠組み。モッジ（E. Moggi）は 1989 年に、例外・状態・非決定性・継続といった「計算の概念」を、値の型 `a` を計算の型 `m a` へ送るモナドとして一様に扱えることを示した。このとき使われた `m`・`return`・`>>=` の組がクライスリトリプルで、`a -> m b` を射と見ればクライスリ圏になる。Haskell が `>>=` を基本に選んでいるのはこの流儀による。ここから、型システムで効果を追跡するエフェクトシステムや代数的効果へつながる。|
...
|Kan 拡張|関手を別の関手に沿って延長する構成。極限・随伴・米田の補題をこれ 1 つで言い換えられるほど一般的で、マックレーンの「すべての概念は Kan 拡張である」で知られる。右 Kan 拡張が `newtype Ran g h a = Ran (forall b. (a -> g b) -> h b)`、左 Kan 拡張が `data Lan g h a = forall b. Lan (g b -> a) (h b)`。`Functor` インスタンスを不要にした Coyoneda は恒等関手に沿った左 Kan 拡張 `Lan Identity f`、上の Codensity は自分自身に沿った右 Kan 拡張 `Ran m m` にあたる。|
