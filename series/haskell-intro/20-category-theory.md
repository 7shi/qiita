...
:::message
Mon は、前に定義した `Mono` とは別物です。👉[一点圏](#一点圏)
...
この Set 側の写像を Mon 側の準同型へ移す 1 対 1 の対応は、$a$ と $m$ について自然であることが求められます。動かしてから移しても、移してから動かしても同じところに着く、ということです。👉[自然性](#自然性)
...
集合 `a` からモノイド `m` への関数を 1 つ与えると、`[a]` から `m` への準同型が一意に決まります。前節で書いた `toMon` がそれです。`Bool` と `Sum Int` に固定していた型を一般化すると、標準ライブラリの `foldMap` になります。
...
要素 1 つの移し方さえ決めれば、並び全体の移し方は決まります。`toMon` の定義が `<>` でつなぐしかなかったのと同じで、他の決め方はありません。
