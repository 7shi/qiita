...
Eff モナドは、複数の効果を 1 つの手順書に混ぜられるようにしたモナドです。命令の型を 1 つに固定せず、型レベルのリストとして持ちます。モナド変換子でモナドを積み重ねていた仕事を、リストへの追加で置き換えます。
...
`es` が具体的なリストではなく型変数のままなことに注目してください。`greet` は「テレタイプとカウンターが入っていればどんなリストでもよい」手順書です。あとから効果を足しても、この宣言は書き換えずに済みます。

## ハンドラー

解釈する側を書きます。前回のインタープリターは手順書を最後まで解釈して結果を返しましたが、効果が複数ある今回は、担当する効果だけを処理して残りは手つかずで返す形になります。効果を 1 つだけ取り除くインタープリターを**ハンドラー**（handler）と呼びます。

型で見ると、リストの先頭の効果が消えます。
...
テレタイプの方は本物の `IO` で処理することにします。こちらはリストが `'[Teletype]` だけになった状態、つまり最後に適用するハンドラーです。
...
最後の行は見慣れない書き方です。`There u'` の `u'` は `Union '[] a` ですが、`Here` も `There` も空でないリストを要求するので、この型の値は作れません。値が存在しない型に対する `case` は、枝を 1 つも書かずに `case u' of {}` と書けます。「ここには来ない」と `error` で書く代わりに、来ないことを型で示した形です。
