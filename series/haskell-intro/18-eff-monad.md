...
ところで、ここまでに書いた型と関数の顔ぶれ、つまり `Eff es a`・`e :> es`・`send`・ハンドラーの型は、中身を丸ごと差し替えても成立します。それを実際にやってみます。
...
前の実装では、`>>=` は `Union` の中身をパターンマッチしてハンドラーの枝を選んでいます。今回はその役目がなく、`IO` の `>>=` に振り替えています。命令をどう解釈するかは、次に見る `send` と `handler` の側に移っています。
...
命令をデータにせず、その場でハンドラーを引いて実行します。

前の実装の `runCounter`・`runTeletype` は、効果ごとに実装した関数で、`u :>>= k` をパターンマッチして、継続 `k` を適用した結果に自分自身を直接呼び直していました。ディスパッチと継続の処理が、効果ごとの関数の中に埋め込まれていたわけです。

今回の `interpret` は、どの効果にも使い回せる汎用の関数です。効果ごとの繰り返し呼び出しは書かず、渡されたハンドラー `f` を環境に積んで返すだけです。積んだハンドラーを実際に呼ぶのは `send`（`handler env op`）の役目で、継続の処理は `Eff` の `Monad` インスタンスが `IO` の `>>=` に委ねて片付けます。ハンドラー自身が呼び出しを重ねる代わりに、`interpret` という汎用の関数を 1 つ間に挟んだ形です。
...
言語機能としてこれを備えた処理系もあります。例えば OCaml 5.0 では `effect` 構文として組み込まれています。
:::
