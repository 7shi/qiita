...
この方式では、組み立てたパーサーが何を受け付けるのかは、実際に文字列を食わせてみるまで分かりません。今回作るのは、合成した時点で受け付ける文字が分かるパーサーです。
...
左が**静的な情報**で、このパーサーが受け付ける文字を並べたものです。右が実際の解析で、解析対象の文字列と入力値を受け取り、成功すれば出力値と残りの文字列を返します。失敗は `Maybe` で表します。
...
合成すると、静的な情報が `t1 ++ t2` で連結されます。解析を実行しなくても、2 つをつないだ結果が何を受け付けるかは決まります。 右側の関数は、前のパーサーの結果と残りを次のパーサーに渡すだけです。
