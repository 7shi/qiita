...
| `c`（`runCont` に渡す受け手） | `a -> r` | 答えを受け取るだけの素の関数 |

`runCont m` が要求するのは `a -> r` の方なので、`k` はそのままでは渡せません。`k x` でモナドを得て、`runCont` で剝がして `c` につなげば型が合います。

```hs:型を合わせた継続
\x -> runCont (k x) c :: a -> r
```

つまり bind は、`k` を `c` と同じ型の継続に変換して `m` に渡す操作です。`Identity` では `k` をその場で呼ぶだけでしたが、`Cont` では変換した継続が関数の形で `Cont` の中に住んでいます。
...
| | 変換するもの | 変換後 | 実装 |
|---|---|---|---|
| bind | `k`（`a -> Cont r b`） | `c` と同じ型 | `\x -> runCont (k x) c` |
